<template>
  ...
  <PsycologistEvaluationsStatics/>
  <StaticRiskLevel/>
</template>

<script>
const API_URL = import.meta.env.VITE_API_BASE_URL;
import PsycologistEvaluationsStatics from '@/components/PsycologistEvaluationsStatics.vue';
...
export default {
  components: { Bar, PsycologistEvaluationsStatics, StaticRiskLevel },
  data() {
    return {
      filters: {
        startDate: '',
  ...
  methods: {
    async fetchPsychologists() {
      try {
        ...
        const response = await axios.get(`${API_URL}/api/users/getpsicologo`, {
          headers: {
            'x-token': token
          }
        });
